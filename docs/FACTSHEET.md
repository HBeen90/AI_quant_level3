...
| 전 테스트 스위트가 통과한다 | 35/35 파일 통과 · 개별 케이스 합계 153 | `tests/run_all.py` | 실측(실행) |
| 실측 백테스트 산출물은 지표 재계산·입력 계보와 자기일관적으로 재현된다 | 레벨 20,881.52 · 누적 19.8815 · CAGR 0.6454 · 변동성 0.4179 · MDD -0.4518 · 원장 해시 일치 · 스냅샷 커밋 3bfa400 | `out/backtest/index_level.csv` | 실측(산출물 자기검증) |
