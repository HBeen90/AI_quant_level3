...
## 검증된 문장 (생성 2026-07-29 · 14/14 재현 성공)
...
| 수시편출의 '정규화 = 제수 흡수'는 두 파트 구현으로 상호 실증됐다 | 최대 상대차 2.18e-15 | `tests/test_index_calc_series.py` | 실측(수치 검증) |
| TR 지수는 제수 보정 경로와 배당 재투자 경로가 일치한다 | 최대 상대차 2.56e-15 | `tests/test_tr_equivalence.py` | 실측(수치 검증) |
...
| 전 테스트 스위트가 통과한다 | 14/14 파일 통과 · 개별 케이스 합계 120 | `tests/run_all.py` | 실측(실행) |
...
| 생존편향 크기 (1.5~3.0%p 등) | 크기 미측정 - 방향·개별 사례 판정만 고지 가능 |
